...
Routes (verified via `pnpm build`): `/` (home), `/about`, `/award-info`, `/countdown`, `/login`,
`/sun-kudos`, `/auth/callback` (route handler), plus the framework's `/_not-found`. Every route
except `/login` and `/auth/**` is gated by `proxy.ts` (Next.js 16's renamed `middleware.ts`) —
