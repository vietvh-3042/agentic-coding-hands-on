...
(`docker exec supabase_db_mock-aidd-kudo-app psql -U postgres -d postgres`) rather than copying
...
- **Total Permission Items**: 15
- **By Type**: route-guard: 2, screen-permission: 0, action-permission: 1, data-permission: 10, role-based: 1, resource-ownership: 0, field-permission: 1, api-scope: 0, feature-flag: 0, experiment: 0, env-gate: 0, locale-gate: 0

_(Note: this tally is recomputed directly from the 15 Index rows above. `permissions.md` previously
carried its own copy of it with `data-permission: 11`, summing to 16 against a stated total of 15;
that duplicate Summary has since been removed — this artifact is now the single tally.)_
...
- [x] All PERM### codes are unique (PERM001–PERM015, contiguous)
