...
- **Entry screen**: SCR001_RootRedirect — `/`
- **Owned screens**:
  - SCR001_RootRedirect — `/` (atomic)
  - SCR002_LoginScreen — `/login` (atomic)
- **Exit screens**: SCR003_CountdownScreen (on successful sign-in, event not yet launched) · SCR004_AboutHomepage (on successful sign-in, event launched) · SCR002_LoginScreen (on sign-out, from anywhere in the app)

### F002_KudosBoardData

- **Entry screen**: SCR006_SunKudosBoard — `/sun-kudos`
- **Owned screens**:
  - SCR006_SunKudosBoard — `/sun-kudos` (composite: bare shell + REG002_SpotlightNameCloud +
    REG003_AllKudosFeed owned outright; REG001_HighlightCarousel shared with F004/F005; sidebar
    stats read-model shared with F006, which owns REG004's secret-box mutation surface)
- **Exit screens**: SCR002_LoginScreen (on session-expired redirect from either auth gate)

### F003_KudoAuthoring

- **Entry screen**: SCR004_AboutHomepage — `/about`
- **Owned screens**:
  - SCR004_AboutHomepage — `/about` (atomic) — widget-button "Write KUDOS" pill + rules-drawer footer button both open the composer
  - SCR006_SunKudosBoard — `/sun-kudos` (composite) — give-kudos bar pill opens the composer
- **Exit screens**: SCR006_SunKudosBoard (on successful submit — board refreshes in place via `router.refresh()`, no navigation)

### F004_KudoHearts

- **Entry screen**: SCR006_SunKudosBoard — `/sun-kudos` _(F004 does not own the bare screen shell — see Owned screens)_
- **Owned screens**:
  - SCR006_SunKudosBoard/REG001 — `/sun-kudos` (region — Highlight carousel heart control; shell + REG002/REG003 owned by F002, filtering owned by F005)
  - SCR006_SunKudosBoard/REG003 — `/sun-kudos` (region — Feed heart control; shell owned by F002, feed data owned by F002)
- **Exit screens**: none — hearting/un-hearting a kudo completes in place on `/sun-kudos`; no navigation away on success or failure.

### F005_HashtagTaxonomy

- **Entry screen**: SCR006_SunKudosBoard — `/sun-kudos` _(F005 does not own the bare screen shell — see Owned screens)_
- **Owned screens**:
  - SCR006_SunKudosBoard/REG001 — `/sun-kudos` (region — Highlight hashtag filter dropdown + card chips; shell + carousel data owned by F002, hearting owned by F004)
  - SCR006_SunKudosBoard/REG003 — `/sun-kudos` (region — Feed hashtag chips + clear-filter chip; shell + feed data owned by F002, hearting owned by F004)
  - SCR006_SunKudosBoard — `/sun-kudos` (bare screen, composer modal instance only — the `KudosHashtagInput` picker inside `KudosFormModal`, shared with F003; F005 owns the picker's catalog/max-5 behavior, not the modal or its submission)
- **Exit screens**: none — selecting, clearing, or composing with a hashtag completes in place on `/sun-kudos` (or the composer modal atop it); no navigation away on any of this feature's own actions.

### F006_SecretBoxReveal

- **Entry screen**: SCR006_SunKudosBoard — `/sun-kudos` _(F006 does not own the bare screen shell — see Owned screens)_
- **Owned screens**:
  - SCR006_SunKudosBoard/REG004 — `/sun-kudos` (region — Sidebar "Open Secret Box" button + the dialog it opens; shell owned by F002)
- **Exit screens**: none — the draw completes in place inside the dialog on `/sun-kudos`; no navigation away on success or failure.

### F007_HomepageOverview

- **Entry screen**: SCR004_AboutHomepage — `/about`
- **Owned screens**:
  - SCR004_AboutHomepage — `/about` (atomic)
- **Exit screens**: SCR005_AwardInfoScreen (hero CTA / award card link / nav link / footer link) · SCR006_SunKudosBoard (hero CTA / kudos promo link / nav link / footer link) · SCR002_LoginScreen (session-expired path via proxy)

### F008_AwardInformationBrowsing

- **Entry screen**: SCR005_AwardInfoScreen — `/award-info`
- **Owned screens**:
  - SCR005_AwardInfoScreen — `/award-info` (atomic)
- **Exit screens**: SCR004_AboutHomepage (nav link / footer link) · SCR006_SunKudosBoard (nav link / footer link / SunkudosSection promo click)

### F009_InterfaceLocalization

- **Entry screen**: N/A — no owned screen; this feature is shared header chrome rendered inside
  screens owned by other features
- **Owned screens**:
  - None — `LanguageSelector` is embedded in `SiteHeader` (login variant and full variant) on
    SCR002_LoginScreen — `/login`, SCR004_AboutHomepage — `/about`, SCR005_AwardInfoScreen —
    `/award-info`, SCR006_SunKudosBoard — `/sun-kudos`; none of these screen shells are owned by
    F009
- **Exit screens**: N/A — switching languages re-renders in place and never navigates

### F010_PrelaunchCountdownGate

- **Entry screen**: SCR003_CountdownScreen — `/countdown`
- **Owned screens**:
  - SCR003_CountdownScreen — `/countdown` (atomic)
- **Exit screens**: SCR004_AboutHomepage (on countdown reaching zero, client-side auto-redirect)

### F011_ProfileSelfService

- **Entry screen**: N/A — no profile screen exists (see technical-spec.md § Absence Evidence)
- **Owned screens**: none
- **Exit screens**: N/A

### F012_InAppNotifications

- **Entry screen**: N/A — renders as header chrome on SCR004/SCR005/SCR006; owns no screen
- **Owned screens**: none
- **Exit screens**: N/A

### F013_AdminRoleGate

- **Entry screen**: N/A — no admin screen exists; the menu item is inert
- **Owned screens**: none
- **Exit screens**: N/A
