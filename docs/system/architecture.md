...
graph TB
    subgraph Browser
        ...
        PROFILE_C["Profile client components: ProfileHero, ProfileBadgeCollection,\nProfileStatsCard, ProfileWriteBar, ProfileKudosSection,\nProfileDirectionDropdown, ProfileFeed"]
    end
    ...
    subgraph Supabase["Supabase - local stack"]
        ...
        SENTVIEW["kudos_sent_for_caller view - security_invoker=off,\nWHERE sender_id = auth.uid() (added 2026-09-07)"]
    ...
    PROFILE_C -->|server-fetch on load| PROFILE_PAGE
    PROFILE_PAGE -->|read via SC: profiles, profile_kudo_stats,\nsecret_box_icons, user_icon_unlocks, kudos| PG
    PROFILE_C -->|direction switch / infinite scroll| PROFILE_SA
    PROFILE_SA -->|received direction| PG
    PROFILE_SA -->|sent direction, no target param| SENTVIEW
    SENTVIEW -->|security_invoker=off + explicit GRANT, not RLS alone| PG
```
...
- **[ADDED 2026-09-07] `kudos_sent_for_caller` closes the anonymous-sender read for one screen,
  not the schema.** The new `/profile` screen's "Sent" feed reads through a `security_invoker=off`
  view — the inverse setting from `profile_kudo_stats` — specifically so it can surface a caller's
  own `sender_id`/`is_anonymous`/`anonymous_name` on rows the base `kudos` table's own permissive
  `"kudos readable by all"` RLS would otherwise let the board's mapper mask. The real boundary is
  the view's `WHERE sender_id = auth.uid()` predicate plus an explicit `REVOKE ALL` + `GRANT SELECT`
  to `authenticated` only. **The base `kudos` table itself is untouched** — narrowing its SELECT
  policy was explicitly out of scope this batch (it would break the live board's own queries
  mid-flight) — so a direct PostgREST call with only the `anon` key can still read any row's
  `sender_id` off `kudos` directly, same as before. Full detail: `permissions.md` Special
  Conditions, `permissions-matrix.md` PERM016.
...
- `supabase/migrations/20260906195000_table_grants_hardening.sql` — the retroactive `REVOKE ALL` +
  ...
  Trust Boundaries.
- `supabase/migrations/20260907010000_profile_read_layer.sql` — **added 2026-09-07**: creates
  `kudos_sent_for_caller` (`security_invoker=off`) + its explicit `REVOKE`/`GRANT SELECT`.
- `app/profile/page.tsx`, `app/profile/actions/load-profile-feed-page.ts` — **added 2026-09-07**:
  the `/profile` Server Component and its one Server Action.
- `lib/profile/resolve-target.ts`, `queries.ts`, `feed-queries.ts`, `hero-tier.ts`, `types.ts` —
  **added 2026-09-07**: the `/profile` read layer (`?id` resolution, header/stats/icon queries,
  received/sent feed pagination, Hero/star tier derivation).
