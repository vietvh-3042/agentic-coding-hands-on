...
### KUDOS_SENT_FOR_CALLER (view, not a table)

**Added**: 2026-09-07, `supabase/migrations/20260907010000_profile_read_layer.sql`, for the new
`/profile` screen's own-sent-Kudos feed. **Not re-verified against a fresh live `psql` session by
this pass** — read directly from the migration file, which is the authoritative source for a view
created and granted in the same migration.

**Description**: `security_invoker = off` (the opposite setting from `profile_kudo_stats` above) —
this is deliberate, not an oversight: the view exists specifically to read past `kudos`' own
permissive `"kudos readable by all"` RLS policy so a caller can see the real `sender_id`/
`is_anonymous`/`anonymous_name` on their own sent rows, which the board's shared mapper would
otherwise mask for anyone but the sender. The confinement is the `WHERE sender_id = auth.uid()`
predicate together with an explicit `REVOKE ALL` + `GRANT SELECT` to `authenticated` only (no
`anon` grant). Full permission detail: `permissions-matrix.md` PERM016.

| Column         | Type        | Description                                                                   |
| -------------- | ----------- | ----------------------------------------------------------------------------- |
| id             | uuid        | = `kudos.id`                                                                  |
| sender_id      | uuid        | Always the caller's own id (enforced by the WHERE clause, not renamed/hidden) |
| receiver_id    | uuid        |                                                                               |
| hashtag_title  | text        | Legacy free-text category column, same as `kudos`                             |
| message        | text        |                                                                               |
| image_urls     | text[]      |                                                                               |
| hearts_count   | integer     |                                                                               |
| is_spam        | boolean     |                                                                               |
| is_anonymous   | boolean     | Not masked here — this is always the caller's own row                         |
| anonymous_name | text        | Not masked here, same reason                                                  |
| created_at     | timestamptz |                                                                               |

**RLS**: N/A — the view itself is the boundary (`security_invoker = off`), not a policy on it.
`REVOKE ALL` then `GRANT SELECT` to `authenticated` only; no `anon` grant.

**Discriminator Fields**: None (mirrors `kudos`' own boolean flags, not re-typed here).
...
- **Total Entities**: 9 tables + 2 views (`profile_kudo_stats`, and `kudos_sent_for_caller` — added
  2026-09-07 for the new `/profile` screen; see its own subsection above)
