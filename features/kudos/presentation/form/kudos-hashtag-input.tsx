...
import { KUDOS_MAX_HASHTAGS } from "@/features/kudos/domain/config";
  ...
  return (
    <div ref={rootRef} className="relative flex flex-wrap items-center gap-2">
      ...
      {open && (
        ...
        >
          {/* An empty master list must still say something: with no rows the
              dropdown would otherwise render as a bare 16px dark pill (just
              its own padding), reading as a stray UI artifact rather than as
          ...
          {hashtags.length === 0 && (
            <li
              ...
              aria-selected={false}
              aria-disabled
              className="px-4 py-3 text-base font-bold text-white/50"
            >
              {t("kudos:hashtag.empty")}
            </li>
          )}
