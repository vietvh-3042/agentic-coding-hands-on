...
import IconPen from "@/shared/ui/icon-pen";
import type { Hashtag } from "@/features/kudos/domain/types";
import { useWidgetStore } from "@/features/homepage/stores/widget-store";
import KudosFormModal from "@/features/kudos/presentation/form/kudos-form-modal";
 ...
 *
 * Client component, so it cannot resolve `getHashtags()` itself — the host
 * page supplies the list. Pages that don't already fetch it render
 * `components/homepage/kudos-widget.tsx` (the async server wrapper) instead
 * of this component directly.
 */
export interface WidgetButtonProps {
  /** Canonical hashtag rows for the write-Kudo modal's picker. */
  hashtags: Hashtag[];
}

export default function WidgetButton({ hashtags }: WidgetButtonProps) {
  const { t } = useTranslation();
  const { isOpen, rulesOpen, kudosOpen, toggleWidget, closeWidget, openRules, closeRules, openKudos, closeKudos } =
    useWidgetStore();
  ...
  useClickOutside(containerRef, closeWidget, isOpen);
  ...
  return (
    <>
      ...
      <SaaRulesDrawer open={rulesOpen} onClose={closeRules} onWriteKudos={handleWriteKudos} />

      <KudosFormModal open={kudosOpen} onClose={closeKudos} hashtags={hashtags} />
    </>
  );
}
