...
import { numberOr, type SupabaseServerClient } from "@/features/kudos/infrastructure/board-query-helpers";
import { heroTierFromDistinctSenders, starTierFromTotalReceived } from "@/features/profile/domain/hero-tier";
import type { ProfileHeader, ProfileIconSlot, ProfileStats } from "@/features/profile/domain/types";

/**
 * Server-only: every export here goes through `createClient()`
 * (`@/shared/infrastructure/supabase/server`), which pulls in `next/headers` — the same
