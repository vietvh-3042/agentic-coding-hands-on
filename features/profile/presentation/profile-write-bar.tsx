...
interface ProfileWriteBarProps {
  ...
  /** Canonical hashtag rows for the modal's picker, resolved by the page
   *  (`app/profile/page.tsx` already fetches them for `ProfileKudosSection`). */
  hashtags: Hashtag[];
}
 ...
 */
export default function ProfileWriteBar({ recipient, hashtags }: ProfileWriteBarProps) {
  ...
  return (
    <>
      ...
      <KudosFormModal open={open} onClose={() => setOpen(false)} hashtags={hashtags} recipient={recipient} />
    </>
  );
}
